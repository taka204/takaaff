...
import { normalizeStatus } from './types.ts'
import type { ConversionFetchOptions, ConversionRecord, ConversionSource } from './types.ts'

/**
 * Đơn hàng lấy qua Shopee Affiliate Open API.
 *
 * CẢNH BÁO: cách Shopee trả subId trong conversionReport chưa xác minh được từ
 * tài liệu công khai. Ở đây giả định chúng nằm trong `utmContent`. Khi được cấp
 * quyền API, việc đầu tiên là in nguyên một bản ghi thật ra rồi sửa
 * `parseSubIds` cho khớp. Cho tới lúc đó đơn vẫn được ghi nhận đầy đủ, chỉ là
 * các chiều phân tích có thể trống.
 */
...
export class ShopeeConversionApiSource implements ConversionSource {
  ...
  async fetchConversions(opts: ConversionFetchOptions): Promise<ConversionRecord[]> {
    ...
    do {
      const data = await graphql<ConversionResponse>(CONVERSION_REPORT_QUERY, {
        purchaseTimeStart: Math.floor(opts.from.getTime() / 1000),
        purchaseTimeEnd: Math.floor(opts.to.getTime() / 1000),
        limit: 500,
      ...
      for (const node of data.conversionReport?.nodes ?? []) {
        ...
        for (const p of node.linkedProductInfo ?? [{}]) {
          records.push({
            ...
            status: normalizeStatus(node.orderStatus ?? ''),
            orderedAt,
            validatedAt: null,
          })
        }
